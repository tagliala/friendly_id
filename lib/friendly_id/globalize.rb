  ...
  module Globalize
    ...
    module SlugGenerator
      ...
      def conflicts
        ...
        sluggable_table_name_quoted = ActiveRecord::Base.connection.quote_table_name(sluggable_class.arel_table.name)
        translation_table_name_quoted = ActiveRecord::Base.connection.quote_table_name(translation_class.arel_table.name)
        base = "#{translation_table_name_quoted}.#{column} = ? OR #{translation_table_name_quoted}.#{column} LIKE ?"
        ...
        scope = scope.where("#{sluggable_table_name_quoted}.#{ActiveRecord::Base.connection.quote_column_name(pkey)} <> ?", value) unless sluggable.new_record?

        length_command = "LENGTH"
        length_command = "LEN" if sluggable.connection.adapter_name =~ /sqlserver/i
        scope = scope.order("#{length_command}(#{translation_table_name_quoted}.#{column}) DESC, #{translation_table_name_quoted}.#{column} DESC")
      end
    end
  end
end
